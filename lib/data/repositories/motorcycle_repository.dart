...
class MotorcycleRepository extends VehicleRepository {
  ...
  @override
  String get vehicleType => 'motorcycle';

  @override
  Future<List<Vehicle>> fetchFromServer() async {
    final response = await apiClient.get(ApiEndpoints.motorcycles);
    ...
    if (response.data['success'] == true) {
      ...
      throw ServerException('Failed to load motorcycles');
    }
  }
  ...
  Future<Vehicle> fetchByIdFromServer(int id) async {
    ...
    if (response.data['success'] == true) {
      ...
      if (vehicle.type.toLowerCase() == 'motorcycle') {
        return vehicle;
      } else {
        throw ServerException('Vehicle is not a motorcycle');
      }
    } else {
      throw ServerException('Motorcycle not found');
    }
  }
}
