...
import 'package:test_task/bloc/cubits/motorcycle_cubit.dart';
...
class MotorcycleDetailsScreen extends StatefulWidget {
  const MotorcycleDetailsScreen({super.key});
  @override
  createState() => _MotorcycleDetailsScreenState();
}
...
class _MotorcycleDetailsScreenState extends State<MotorcycleDetailsScreen>
  ...
  Widget build(BuildContext context) => BlocProvider(
    create: (context) => MotorcycleCubit()..loadMotorcycles(),
    child: Scaffold(
      backgroundColor: BaseColors.background,
      body: BlocBuilder<MotorcycleCubit, MotorcycleState>(
        builder: (context, state) {
          if (state is MotorcycleInitial) {
            return Center(child: CircularProgressIndicator());
          }
          if (state is MotorcycleLoaded) {
            return CustomBackgroundWithGradient(
              child: Column(
                children: [
                  ...
                  MotorcyclePage(cars: state.motorcycles),
                  SizedBox(height: 24),
                  _buildDateSelector(),
                  SizedBox(height: 24),
                  _buildBookingPanel(),
...
class MotorcyclePage extends StatefulWidget {
  late List<Motorcycle> cars;
  MotorcyclePage({super.key, required this.cars});

  @override
  createState() => _CarPageState();
}

class _CarPageState extends State<MotorcyclePage>
    with TickerProviderStateMixin {
  int currentIndex = 0;
